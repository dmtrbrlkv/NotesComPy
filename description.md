...
Поправим значение справочника уровня Junior на Beginer и обновим сортудников:
```
from notescompy import init_session, open_database
from notescompy.extended_collection import ExtendedCollection
s = init_session("python")
...
#### Логирование
Подерживаается логирование в базу логов и в файл. Дополнительно реализованы уровни логирования DEBUG, INFO, ERROR.
...
Напишем на Python функцию, актуализирующу сотрудников аналогично агенту. На вход передеается измененный документа справочника, по представлению ищутся сотрудники, у которых указано значение измененного справочника, значение актуализируется
Lotus Script:
```
Sub Initialize
...
В скрипте инициализируем сессию текущей сессией клиента, получим переменные окружения и вызовим функцию обновления сотрудников
